...
export function 逆向解析舱() {
  // 预览画布显示尺寸（像素）/ Preview canvas display size (px)
  const 预览尺寸 = 320;
  ...
  // 非 .pnt 文件被拒绝时提示 / Notify when non-.pnt files are rejected
  const onDropRejected = useCallback(() => {
    toast.error('仅支持 .pnt 文件', {
      description: '请上传方舟绘画 .pnt 格式文件',
    });
  }, []);

  // .pnt 无标准 MIME 类型，不设 accept 限制，改为在 onDrop 中按扩展名校验
  // .pnt has no standard MIME type, omit accept and validate by extension in onDrop
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    noClick: false,
    validator: (文件: File) => {
      const 是Pnt = 文件.name.toLowerCase().endsWith('.pnt');
      if (!是Pnt) {
        return {
          code: 'not-pnt',
          message: '仅支持 .pnt 文件 / Only .pnt files are supported',
        };
      }
      return null;
    },
  ...
  useEffect(() => {
    ...
    // 解析结果.像素 已是 RGBA 格式（由引擎 索引转RGBA 生成），可直接绘制
    // Parse result pixels are already RGBA (generated by engine's 索引转RGBA), can draw directly
  ...
  const 处理鼠标移动 = useCallback(
    (event: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      // 直接通过 Color ID 索引查找颜色（比 RGB 匹配更精准）
      // Look up color directly via Color ID index (more accurate than RGB matching)
      const 像素索引 = y * 解析结果.宽度 + x;
      const 颜色ID = 解析结果.索引.数据[像素索引];
