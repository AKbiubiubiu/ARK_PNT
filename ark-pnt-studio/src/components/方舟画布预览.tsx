    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-zinc-300">方舟画布预览</span>
        </div>
        {像素矩阵 && (
    ...
    <div className="rounded-xl bg-black/20 p-3 ring-1 ring-white/5">
      ...
      <div className="flex flex-wrap gap-1.5">
        {染料列表.map(({ 染料, 数量 }) => (
          <div
            key={染料!.编号}
            className="group relative flex items-center gap-1.5 rounded-full bg-white/5 px-2 py-1 ring-1 ring-white/10"
          ...
          >
            ...
            />
            <span className="text-[10px] text-zinc-300">{染料!.中文名}</span>
          </div>
        ))}
      </div>
    </div>
