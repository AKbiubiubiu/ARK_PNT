...
export function 方舟画布预览({ 像素矩阵, 显示尺寸 = 256 }: 方舟画布预览属性) {
  ...
  const 处理鼠标移动 = useCallback(
    (event: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (A === 0) {
        set悬停染料({
          染料: { 编号: 0, 中文名: '透明', 英文名: 'Transparent', 红: 0, 绿: 0, 蓝: 0, 透明: true },
      ...
      // 在调色板中查找匹配染料（通过 RGB 反查）/ Find matching dye via RGB lookup
      const 匹配染料 = 方舟颜色列表.find(
        (d) => d.红 === R && d.绿 === G && d.蓝 === B
      );
      if (匹配染料) {
        set悬停染料({
...
function 染料使用统计({ 像素矩阵 }: { 像素矩阵: 像素矩阵类型 }) {
  const 统计 = useCallback(() => {
    ...
    // 转换为染料列表 / Convert to dye list
    const 列表 = Array.from(计数.entries())
      .map(([键, 数量]) => {
        const [R, G, B] = 键.split(',').map(Number);
        const 匹配染料 = 方舟颜色列表.find((d) => d.红 === R && d.绿 === G && d.蓝 === B);
        return { 染料: 匹配染料, 数量 };
      })
      .filter((项) => 项.染料)
      .sort((a, b) => b.数量 - a.数量);
